...
class _HotelPageState extends State<HotelPage>
  ...
  /*Future getData() async {
    ...
    if (response.features == null) {
      ...
      for (var i = 0; i < response.features!.length; i++) {
        Hotel d = Hotel(
            response.features![i]!.textEs!,
            response.features![i]!.placeName!,
            response.features != null
                ? response.features![i]!.center![1]!.toDouble()
                : 0.0,
            response.features != null
                ? response.features![i]!.center![0]!.toDouble()
                : 0.0,
            4,
            0);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.placeData!.nombre.toString(),
